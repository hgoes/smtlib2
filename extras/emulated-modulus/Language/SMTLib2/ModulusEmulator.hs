...
toAssert (EmulatedMod res resVar mod modE expr) b = do
  (diff,b1) <- toBackend (App (Arith NumInt Minus (Succ (Succ Zero))) (Cons expr (Cons resVar Nil))) b
  (div,b2) <- toBackend (App (Divisible mod) (Cons diff Nil)) b1
  (c0,b3) <- toBackend (Const (IntValue 0)) b2
  (lt,b4) <- toBackend (App (Ord NumInt Lt) (Cons resVar (Cons modE Nil))) b3
  (ge,b5) <- toBackend (App (Ord NumInt Ge) (Cons resVar (Cons c0 Nil))) b4
  return ([div,lt,ge],b5)
toAssert (EmulatedDiv res expr div diff) b = do
  (resVar,b1) <- toBackend (Var res) b
  (diffVar,b2) <- toBackend (Var diff) b1
  (prod,b3) <- toBackend (App (Arith NumInt Mult (Succ (Succ Zero))) (Cons resVar (Cons div Nil))) b2
  (diff',b4) <- toBackend (App (Arith NumInt Minus (Succ (Succ Zero))) (Cons expr (Cons diffVar Nil))) b3
  (eq,b5) <- toBackend (App (Eq IntRepr (Succ (Succ Zero))) (Cons prod (Cons diff' Nil))) b4
  (c0,b6) <- toBackend (Const (IntValue 0)) b5
  (ge,b7) <- toBackend (App (Ord NumInt Ge) (Cons diffVar (Cons c0 Nil))) b6
  (lt,b8) <- toBackend (App (Ord NumInt Lt) (Cons diffVar (Cons div Nil))) b7
  return ([eq,ge,lt],b8)
...
instance (Backend b) => Backend (ModulusEmulator b) where
  ...
  toBackend (App (ArithIntBin Mod) (Cons x (Cons y Nil))) me
    ...
    Const (IntValue y') -> do
      ...
      (res,b3) <- if y'>=0
                  then return (resE,b2)
                  else toBackend (App (Arith NumInt Minus (Succ Zero)) (Cons resE Nil)) b2
      return (res,addEmulation (EmulatedMod resVar resE y' y x)
                  (me { emulatedBackend = b3 }))
  toBackend (App (ArithIntBin Rem) (Cons x (Cons y Nil))) me
    = case fromBackend (emulatedBackend me) y of
    Const (IntValue y') -> do
      (resVar,b1) <- declareVar IntRepr Nothing (emulatedBackend me)
      (resE,b2) <- toBackend (Var resVar) b1
  ...
  toBackend (App (ArithIntBin Div) (Cons x (Cons y Nil))) me = do
    (resVar,b1) <- declareVar IntRepr Nothing (emulatedBackend me)
  ...
  fromBackend me e = case fromBackend (emulatedBackend me) e of
    Var v -> case [ (modE,expr,refl) | EmulatedMod res resE mod modE expr <- emulations me
             ...
             (cmod,expr,Refl):_ -> App (ArithIntBin Mod) (Cons expr (Cons cmod Nil))
             [] -> case [ (expr,div,diff,refl) | EmulatedDiv res expr div diff <- emulations me
                                               , Just refl <- [geq v res] ] of
                   (expr,div,diff,Refl):_ -> App (ArithIntBin Div) (Cons expr (Cons div Nil))
                   [] -> case [ (res,expr,div,refl)
                              | EmulatedDiv res expr div diff <- emulations me
                              , Just refl <- [geq v diff] ] of
                     (res,expr,div,Refl):_ -> App (ArithIntBin Mod) (Cons expr (Cons div Nil))
                     [] -> Var v
    res -> res
